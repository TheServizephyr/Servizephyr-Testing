...
function getOrderCustomerIdentity(orderData, orderDocId, customerLookupById, customerLookupByPhone) {
  const candidateIds = [orderData.customerId, orderData.userId]
    .filter(Boolean)
    .map(String);
...
function computeLeaderboardMetrics(customers, ordersDocs, weekStart, weekEnd) {
  ...
  ordersDocs.forEach((doc) => {
    ...
    if (nonCountableStatuses.has(normalizedStatus)) return;
  ...
  allMetrics.forEach((metric) => {
    ...
    if (metric.orderDates.length >= 2) {
      let totalGapHours = 0;
      for (let i = 1; i < metric.orderDates.length; i += 1) {
  ...
  const maxInverseGap = Math.max(
    0.000001,
    ...allMetrics.map((m) => (m.avgGapHours > 0 ? 1 / m.avgGapHours : 0))
  );
...
async function getOwnerCustomers(req) {
  ...
  let customersSnap;
  try {
    customersSnap = await owner.businessSnap.ref.collection('customers').orderBy('totalSpend', 'desc').get();
  } catch {
    customersSnap = await owner.businessSnap.ref.collection('customers').get();
  }
  ...
  const topSpender =
    customers.length > 0
      ? customers.reduce((prev, current) =>
        (prev.totalSpend || 0) > (current.totalSpend || 0) ? prev : current
      )
      : {};
  const topOrderer =
    customers.length > 0
  ...
  const newThisMonth = customers.filter((customer) => {
    const joinOrLast = toDate(customer.joinedAt) || toDate(customer.lastOrderDate);
  ...
  const loyalCustomers = customers.filter((customer) => customer.statusTag === 'Loyal').length;
  const atRiskCustomers = customers.filter((customer) => customer.statusTag === 'At Risk').length;
  const inactiveCustomers = customers.filter((customer) => {
    const last = toDate(customer.lastOrderDate);
    if (!last) return true;
  ...
  const [thisWeekOrderDocs, prevWeekOrderDocs] = await Promise.all([
    queryOrdersByRange(owner.firestore, owner.businessId, weekStart, weekEnd),
    queryOrdersByRange(owner.firestore, owner.businessId, prevWeekStart, prevWeekEnd),
  ]);
  ...
  const stats = {
    ...
    weeklyRevenue: Number(
      currentWeekBoard.pointTable.reduce((sum, row) => sum + (row.weeklySpend || 0), 0).toFixed(2)
    ),
    previousWeekOrders: previousWeekBoard.pointTable.reduce((sum, row) => sum + (row.weeklyOrders || 0), 0),
    previousWeekRevenue: Number(
      previousWeekBoard.pointTable.reduce((sum, row) => sum + (row.weeklySpend || 0), 0).toFixed(2)
    ),
  };
  ...
  return { customers, stats, leaderboard };
}

async function patchOwnerCustomer(req, body = {}) {
  const owner = await resolveOwnerContext(req, {
    checkRevoked: true,
    requiredPermissions: [PERMISSIONS.VIEW_CUSTOMERS],
  });
  ...
  if (!customerId || !hasNotes) {
    throw new HttpError(400, 'Customer ID and notes are required.');
  }

  const customerRef = owner.businessSnap.ref.collection('customers').doc(customerId);
  const customerSnap = await customerRef.get();
  if (!customerSnap.exists) {
    throw new HttpError(404, 'Customer not found in this business.');
  }

  await customerRef.update({ notes: body.notes });
