...
function detectPaymentBucket(order = {}) {
...
function computeStreaks(dateKeysSet) {
...
async function getCustomerAnalytics(req) {
  ...
  const [userDoc, userIdOrdersSnap, legacyOrdersSnap, joinedRestaurantsSnap] = await Promise.all([
    ...
    orderTimeline.push({
      ...
      items: items.slice(0, 5).map((item) => ({
        ...
        spend: Number(
          Math.max(
            0,
            toAmount(item?.totalPrice, toAmount(item?.price) * Math.max(1, toAmount(item?.quantity ?? item?.qty, 1)))
          ).toFixed(2)
        ),
      })),
    });
  ...
  const userLoyaltyPoints = Math.max(0, toAmount(userData.loyaltyPoints, loyaltyTotalFromRestaurants));
